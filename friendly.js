...
World.prototype = {      
  ...
  stuff_collide_: function(p1, p2, check_walls, check_items) {
    ...
    if(check_items) {
      for(var i=0,n=this.agents.length;i<n;i++) {
        var a = this.agents[i];
        var res = line_point_intersect(p1, p2, a.p, a.rad);
        if(res) {
          res.type = 1; // store type of item
          res.vx = a.v.x; // velocty information
          res.vy = a.v.y;
          if(!minres) { minres=res; }
          else { if(res.ua < minres.ua) { minres = res; }
          }
  ...
  tick: function() {
    ...
    for(var i=0,n=this.items.length;i<n;i++) {
      var it = this.items[i];
      var nearest_agent = new Vec(99999,99999); // haven't seen any agents yet..don't move
      it.age += 1;
      ...
      for(var j=0,m=this.agents.length;j<m;j++) {
        ...
        if(d < it.rad + a.rad) {
          ...
          // find vector from item to nearest agent
          if(a.p.sub(it.p).length() < nearest_agent.length()) {
            nearest_agent = a.p.sub(it.p);
          }
        }
      }
    ...
    if(this.items.length < this.nitems && this.clock % 10 === 0 && randf(0,1)<0.25) {
      var items_to_replace = 5; //Math.floor((this.nitems - this.items.length)/7);
